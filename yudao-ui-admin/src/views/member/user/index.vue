...
<!--      </el-form-item>-->
      <el-form-item label="用户角色" prop="role">
        <el-select v-model="queryParams.role" placeholder="请选择用户角色" clearable size="small">
          <el-option v-for="dict in this.getDictDatas(DICT_TYPE.MEMBER_USER_ROLE)"
                       :key="dict.value" :label="dict.label" :value="dict.value"/>
        </el-select>
      </el-form-item>
<!--      <el-form-item label="微信openid" prop="openid">-->
<!--        <el-input v-model="queryParams.openid" placeholder="请输入微信openid" clearable @keyup.enter.native="handleQuery"/>-->
<!--      </el-form-item>-->
...
<!--      <el-table-column label="头像" align="center" prop="avatar" />-->
      ...
      <el-table-column label="用户角色" align="center" prop="role">
        <template v-slot="scope">
          <dict-tag :type="DICT_TYPE.MEMBER_USER_ROLE" :value="scope.row.role" />
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      const id = row.id;
      this.$modal.confirm('是否确认删除用户登录表编号为"' + id + '"的数据项?').then(function() {
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有用户登录表数据项?').then(() => {
          this.exportLoading = true;
          return exportUserExcel(params);
        }).then(response => {
          this.$download.excel(response, '用户登录表.xls');
